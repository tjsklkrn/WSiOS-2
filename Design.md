...
│  │ (JWT middleware) │──▶️│  routes/registry.js              │   │
│  └──────────────────┘   └──────────────┬─────────────────-─┘   │
└────────────────────────────────────────┼────────────────────────┘
                                         │
              ┌──────────────────────────┼──────────────────────┐
...
│   ├── pineconeService.js      # embedding generation + Pinecone query
...
- **Dimension:** 1536 (OpenAI `text-embedding-3-small`) or 768 (alternative model)
- **Metric:** cosine
...
scripts/seedPinecone.js:
  ...
  3. Call OpenAI Embeddings API (or local model) to get vector
  4. Upsert to Pinecone in batches of 10
...
async function queryForCart(cartProductIds) {
  ...
  for (const productId of cartProductIds) {
    ...
    const queryVector = await getEmbedding(embeddingText);   // cached per productId
    ...
    response.matches.forEach(match => {
      if (match.id !== productId) {   // exclude self
        results.push({ productId: match.id, score: match.score, source: "pinecone" });
      }
    });
  }
  return results;
...
| `OPENAI_API_KEY` | OpenAI API key for embedding generation (seed script + query time) |
