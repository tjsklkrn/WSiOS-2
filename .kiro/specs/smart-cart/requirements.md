...
**Smart Cart** introduces a heterogeneous product relationship graph built from `skus.json`, a parallel recommendations engine combining graph traversal and Pinecone vector search, bundle detection with registry-style category grouping, save-for-later surfacing for unavailable products, and Firebase Auth token validation. The server returns structured recommendation data only; any on-device copy generation is handled separately in Swift and is out of scope for this backend spec.
...
### Requirement 8: Cart API Endpoints
...
7. WHEN a `productId` supplied to any cart endpoint does not exist in `skus.json`, THEN THE Smart Cart API SHALL return HTTP 404 with an error body identifying the unknown product ID.
8. THE Smart Cart API SHALL return all responses as JSON with `Content-Type: application/json`.
9. THE Smart Cart API SHALL include a `totalPrice` field (sum of `sellingPrice × quantity` for all active cart items) and a `totalItems` field (sum of all active item quantities) in every cart state response.
